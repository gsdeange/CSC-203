...
public class PathingMain extends PApplet
{
    ...
    private boolean moveOnce(Point pos) {
        ...
        if(withinBounds(pos, grid) && grid[pos.x][pos.y] != GridValue.OBSTACLE && grid[pos.x][pos.y] != GridValue.SEARCHED)
        {
            // if within grid, not searched and not an obstacle

            if(grid[pos.x][pos.y] == GridValue.GOAL) // check if we reached the end
            {
                reachedgoal = true;
            }
            else
            {
                grid[pos.x][pos.y] = GridValue.SEARCHED;
                reachedgoal = moveOnce(tryNodes[0]) || moveOnce(tryNodes[1]) || moveOnce(tryNodes[2]) || moveOnce(tryNodes[3]);
            }
        }
        if(reachedgoal == true)
        {
