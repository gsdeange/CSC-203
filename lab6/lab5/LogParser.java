...
public class LogParser 
{
    ...
    private void processStartEntry(String[] words) throws ParseException 
    {
        ...
        {
            throw new ParseException(inputFile, lineNumber, "start tag with " + words.length + " words");
        }
        // TODO:  Get the right Customer object

        Function<Customer> createCustomer =  () -> {
        ...
        //final Customer customer = log.customerbyID.compute(customerid, )
        final Customer customer = log.customerbyID.compute(customerid, createCustomer);
        ...
        //
        Function<String, Customer, Session> createSession =  (id, customer) -> {
        ...
        Session session = log.sessionByID.computeIfAbsent(sessionID, createSession(sessionID, customer));
        if (session.customer != customer) 
        {
            //
            // NOTE:  You're not required to do this kind of validation,
            // but you're welcome to.  Sometimes checking file integrity
            // can help catch bugs.  This is mostly being checked here to
            // reinforce the existence constraint in our specification.
            //
            throw new ParseException(inputFile, lineNumber, "customer " + customer + " != " + session.customer);
        }
    ...
    private void processEndEntry(final String[] words) throws ParseException 
    {
        ...
        {
            throw new ParseException(inputFile, lineNumber, "end tag with " + words.length + " words");
        }
        Session currentSession = log.sessionByID.get(word[END_SESSION_ID]);
